...
namespace LINQ
{
    public partial class frmEmployeeGrid : Form
    {
        ...
        private void btnSelectNameSurname_click(object sender, EventArgs e) {
            var employeeList = GetEmployeeList();
            SetDataIntoGrid(employeeList.Where(x => (x.Name.ToLower().Contains("i") && x.Surname.ToLower().Contains("i"))
            || (x.Name.ToLower().Contains("e") && x.Surname.ToLower().Contains("e")))
            .OrderByDescending(x=> x.Surname).ThenBy(x=>x.Name).ToList());
        ...
        private void btnSelectMultiple_click(object sender, EventArgs e) {
            var employeeList = GetEmployeeList();
            SetDataIntoGrid(employeeList.Where(x => x.Name.ToLower()
            .Intersect(x.Surname.ToLower())
            .Intersect(x.FacultyName.ToLower())
            .Count() > 1).ToList());
       }
        ...
        private void btnSelectCompareLength_click(object sender, EventArgs e) {
            var employeeList = GetEmployeeList();
